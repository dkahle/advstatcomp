...
library(readr)
library(dplyr)
dat <- read_csv("daily_44201_2016.csv.bz2")
names(dat) <- make.names(names(dat))
o3 <- filter(dat, State.Code == "06" & County.Code == "037" 
             & Site.Num == "0002") %>%
        select(Date.Local, Arithmetic.Mean) %>%
        rename(ozone = Arithmetic.Mean, date = Date.Local) %>%
...
library(readr)
library(ggplot2)
o3 <- read_csv("los_angeles_ozone.csv") %>%
        filter(!is.na(ozone))
ggplot(o3, aes(x = ozone)) + 
        geom_histogram(bins = nclass.Sturges(o3$ozone))
```


```{r}
nll_one <- deriv(~ -log(dnorm((x - mu)/s) / (pnorm((x-mu)/s) - pnorm(-mu/s))),
                 c("mu", "s"), function.arg = TRUE)
nll <- function(p) {
        v <- nll_one(p[1], p[2])
        gr <- colSums(attr(v, "gradient"))
        structure(sum(v), gradient = gr)
}
```
