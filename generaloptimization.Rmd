...
The maximum likelihood estimate of $\mu$ is `r res$par[1]` and the estimate of $\sigma$ is `r res$par[2]`. If we wanted to obtain asymptotic standard errors for these parameters, we could look at the Hessian matrix.
...
Commonly, we see that this model is fit using more complex algorithms like the EM algorithm or Markov chain Monte Carlo methods. While those methods do provide greater stability in the estimation process (as we will see later), we can in fact us Newton-type methods to maximize the likelihood (with a little care). 
